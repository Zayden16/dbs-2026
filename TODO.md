...
- [x] **Kap. 5 metric definition:** "ICE Kosten: … × Kraftstoffpreis des Landes" (singular) — actual = mean of petrol & diesel. Reword to `(Benzin+Diesel)/2`.

## P1 — Stale numbers / inaccurate mechanism

- [x] **Kap. 4.3.1 currency claim false:** "Umrechnung aller vorliegenden Preise in CHF" — prices are in EUR (cost/km labelled €/km). Remove or correct.
...
- [x] **Kap. 4.3.1 fuel-price mechanism:** says values "aus fuel_prices.csv extrahiert … in country eingefügt". SQL loader sets them as `INSERT … VALUES` constants (now matching the CSV); MongoDB reads `stg_fuel_prices`. Note this honestly (SQL has no `stg_fuel_prices`).
- [x] **Abb. 13 / Abb. 14 Mongo counts:** verify "vehicles: 8'670, country_year_data: 78, charging_stations: 38'071" against live DB; update if changed after re-transform.
- [x] **Kap. 4.1.2 Zugangsdaten:** MySQL table says `Database: railway` but connection string ends `/ev_project`. Use `ev_project` consistently (the ILIAS correction email). NOTE: live credentials are printed in the report — required for the "checkable online system", but they're in the repo `.env` (gitignored), do NOT also commit them in any md.

## P2 — Placeholders / structure (Kap. 7, Corsin's part)

- [x] **Kap. 7.1.4 Action** is `[TODO]` — fill: EV recommended in CH/DE/FR (ratios), plus a 1-line sensitivity note (how recommendation shifts if electricity/fuel price changes).
- [x] **Kap. 7.1.5 "(fix title) Personas"** empty — resolve each persona (Stefan/CH, Lisa/DE, Marc/FR) with its `mv_ev_decision` line, or trim the 3-persona framing in Kap. 2.4.
...
- [x] **Kap. 1 / 2.1 "TCO" framing vs delivery:** Kap. 2 promises full LCA, break-even, segments, subsidies, green-charging, maintenance, depreciation — none delivered (analysis = operating cost/km + grid CO₂). Add one "Abgrenzung/Scope" sentence narrowing it, or move the unbuilt items to "Ausblick".
- [x] **Kap. 8.2 TODO** lists future use cases (break-even, charging density, EV adoption) — decide keep-as-ausblick vs build (break-even is feasible: have grid CO₂ + consumption; only the battery CO₂-backpack input is missing).
- [x] Title page typos: "Real Team Name 1", "Corsin Hidbeer" (→ Hidber).
